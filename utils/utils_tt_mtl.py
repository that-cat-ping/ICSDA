...
def collate_MIL(batch):
	...
	pt = torch.LongTensor([item[2] for item in batch])
 
	pn = torch.LongTensor([item[3] for item in batch])
	pm = torch.LongTensor([item[4] for item in batch])
	stage = torch.LongTensor([item[5] for item in batch])
	OTOS = torch.LongTensor([item[6] for item in batch])
	LCE3E = torch.LongTensor([item[7] for item in batch])
	PCDHA2 = torch.LongTensor([item[8] for item in batch])
	METTL8P1 = torch.LongTensor([item[9] for item in batch])
	LINC02124 = torch.LongTensor([item[10] for item in batch])
	# gender = torch.LongTensor([item[2] for item in batch])
	# return [img, label, gender...]
	return [img, label,pt,pn,pm,stage,OTOS,LCE3E,PCDHA2,METTL8P1,LINC02124]
