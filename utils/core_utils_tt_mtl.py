import numpy as np
import torch
from utils.utils_tt_mtl import *
import os
from datasets.dataset_generic_tt_mtl import save_splits  
# from models.model_mil import MIL_fc, MIL_fc_mc
from models.model_clam_tt_mtl import TOAD_mtl  
...
def train_loop(epoch, model, loader, optimizer, n_classes, datasize, writer = None, loss_fn = None,):
    ...
    for batch_idx, (data, label,pt,pn,pm,stage,OTOS,LCE3E,PCDHA2,METTL8P1,LINC02124) in enumerate(loader):
        data, label = data.to(device), label.to(device)
        pt = pt.float().to(device)
        pn = pn.float().to(device)
        pm = pm.float().to(device)
        stage = stage.float().to(device)
        OTOS = OTOS.float().to(device)
        LCE3E = LCE3E.float().to(device)
        PCDHA2 = PCDHA2.float().to(device)
        METTL8P1 = METTL8P1.float().to(device)
        LINC02124 = LINC02124.float().to(device)
        # label = label.float().to(device)
        # gender = gender.to(device)

        optimizer.zero_grad()
        results_dict = model(data,pt,pn,pm,stage,OTOS,LCE3E,PCDHA2,METTL8P1,LINC02124)
...
def summary(model, loader, n_classes):
    ...
    if n_classes == 2:
        ...
        # ROC曲线
        fpr, tpr, thresholds = roc_curve(np.array(all_labels), np.array(all_probs)[:, 1])
        roc_auc = auc(fpr, tpr)
        ...
        plt.show()
        fig.savefig('/media/zw/Elements1/lvyp/results/metastasis_vs_normal_mtl_1_s5/resnet18_roc_k1_with_clinical.svg')
